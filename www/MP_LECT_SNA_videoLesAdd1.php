...
//	$subTopic=Pulisci_INS($_POST["subtopic"]);
	...
	if ($checkStatus) {
		...
		else $redirectUrl="./MP_LECT_SNA_videoLesAdd2.php?idVideo=".$vidId; // passa alla scelta delle keywords 

	} else $redirectUrl="./MP_LECT_SNA_videoLesAdd1.php?idVideo=".$vidId."&msg=KO&from=".$fromPage;
...
?>
					...
					<form method="post" action="./MP_LECT_SNA_videoLesAdd1.php?act=reg&idVideo=<?=$vidId?>&from=<?=$fromPage?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
						...
						<div style="padding-top: 25px;">
							...
							<div style="width: 625px;padding: 10px 0 10px 10px;border: dotted 1px #00aeef;border-radius: 5px;">
								...
								while ($row=mysqli_fetch_array($result)) { 
									...
									$totSubTop=mysqli_num_rows($result11);
									...
									while ($row1=mysqli_fetch_array($result1)) { 
										...
										?><div style="padding: 0 0 0 25px;"><input type="checkbox" name="subtopic<?=$subtopicId?>" value="<?=$subtopicId?>" <?php if (strrpos($videoSubTopic,"_".$subtopicId."_")) echo "checked=\"checked\"";?> /> <label><?=$subtopicName?></label></div><?php 
									}
						...
						<?php if ($fromPage=="edit") {?>
							<div class="signup_submit" style="width: 240px;margin: 25px auto;">
								<a href="./MP_LECT_SNA_videoLesEdit.php?from=<?=$fromPage?>&id_act=<?=$vidId?>"><button type="button" class="abort" style="margin: 0 0 0 0;font-size: 1.5em;padding: 5px 20px;" />Abort</button></a>
						...
						<?php } else {?>
							<div class="signup_submit" style="width: 240px;margin: 25px auto;">
								<a href="./MP_LECT_SNA_videoLesManage.php<?=$queryStr?>"><button type="button" class="abort" style="margin: 0 0 0 0;font-size: 1.5em;padding: 5px 20px;" />Abort</button></a>
