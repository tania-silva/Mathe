...
if (isset($_GET["assId"])) $assId=$_GET["assId"];
if (isset($_GET["qstNb"])) $qstNb=$_GET["qstNb"];
...
?>
	...
	<script src='https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.5/latest.js?config=TeX-MML-AM_CHTML' async></script>
					...
					<p class="rsvPage_Title">Select Question</p>
					<p class="rsvPage_Title1">Student Final Assessment</p>

					<?php 
					$where=" WHERE (((p.id_lect=$usrId AND p.validate=0) OR p.validate=1) AND ";
					...
					<div class="signup_submit" style="padding: 0 0 10px 0;">
						<p style="float: left;width: 300px;padding: 50px 10px 0 0;">Found <?=$totale?> questions</p>
						<p style="float: right;width: 113px;padding: 0 10px 0 0;"><a href="./MP_LECT_SFA_assessmentNew1.php?assId=<?=$assId?>"><button type="button" class="abort" style="width: 100px;padding: 5px;" />Exit</button></a></p>
						<div class="clear"></div>
					</div>
					...
					<form method="post" action="./MP_LECT_SFA_assessmentNew2.php?assId=<?=$assId?>&qstNb=<?=$qstNb?>" style="padding: 10px;background-color: #e1f7ff;border: solid 1px #00AEEF;border-radius: 5px;">
						...
						<div style="float: left;width: 190px;">
							...
							<select name="srcValidation">
								...
								<option value="3" <?php if ($srcValidation==3) echo "selected";?>>YOUR QUESTIONS</option>
								<!-- <option value="4" <?php if ($srcValidation==4) echo "selected";?>>WAITING FOR VALIDATION</option> -->
								<option value="1" <?php if ($srcValidation==1) echo "selected";?>>MATHE DATABASE</option>
						...
						<div style="float: right;width: 100px;margin-right: 10px;">
							<a href="./MP_LECT_SFA_assessmentNew2.php?assId=<?=$assId?>&qstNb=<?=$qstNb?>"><button type="button" class="abort" />All</button></a>
					...
					<table class="table table-hover">
						...
						while ($row=mysqli_fetch_array($resultP)) { 
							...
							$lnk1="./MP_LECT_SFA_questionEdit.php?id_act=".$qstId;
							$lnk3="./MP_LECT_SFA_questionDelete.php?id_act=".$qstId;
							...
							$attach=0;
							$pict="./data/mathePlatform/SFA/attach/{$qstId}.{$fileExt}";
							...
							<tr>
								<td style="font-size: 0.9em;">
									...
									<p style="padding-top: 5px">Code:<br /><strong>SFA<?=$qstId?></strong></p>
								...
								<td style="text-align: right;">
									<div style="width: 90px;margin: 0 0 15px 5px;">
										<div style="width: 90px;padding: 10px 5px;font-weight: 600;color: #090;text-align: center;border: solid 1px #090;">
											<p style=""><a href="./MP_LECT_SFA_assessmentNew1.php?assId=<?=$assId?>&qstNb=<?=$qstNb?>&act=reg&qstId=<?=$qstId?>" style="font-size: 0.8em;line-height: 0.5em;color: #090;">SELECT FOR FINAL ASSESSMENT</a></p>
										</div>
									</div>
					...
					<!-- <?php if ($pagine>1) {?>
						<div class="pageNav">
							<?php 
							for ($i=1;$i<=$pagine;$i++) {
								$str_query="&srcLevel=".$srcLevel."&srcTopic=".$srcTopic."&srcValidation=".$srcValidation."&assId=".$assId."&qstNb=".$qstNb;
					...
					<?php }?> -->
					<?php if ($pagine>1) include('./impianto/paginazione1.php'); //paginazione?>
