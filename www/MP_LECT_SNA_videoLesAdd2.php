...
if ($_GET["act"]=="reg" AND $usrId) {
	...
	if ($checkStatus) {
		...
		$sqlDelete = " 
		DELETE FROM `platform_keyword_videolessons` WHERE `id_video_lesson` = '$vidId'";
		$resultDelete=mysqli_query($conn,$sqlDelete);

		$sqlInsert= "
		INSERT INTO `platform_keyword_videolessons`(`id_keyword`, `id_video_lesson`) VALUES";
		...
		else $redirectUrl="./MP_LECT_SNA_videoLesAdd3.php?idVideo=".$vidId; // passa alla scelta delle questions 

	} else $redirectUrl="./MP_LECT_SNA_videoLesAdd2.php?idVideo=".$vidId."&msg=KO&from=".$fromPage;
...
?>
					...
					<form method="post" action="./MP_LECT_SNA_videoLesAdd2.php?act=reg&idVideo=<?=$vidId?>&from=<?=$fromPage?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
						<?php var_dump($keywords);?>
						<p>Please select keywords for this video lessons</p>
						...
						<?php } else {?>
							<div class="signup_submit" style="width: 240px;margin: 25px auto;">
								<a href="./MP_LECT_SNA_videoLesManage.php"><button type="button" class="abort" style="margin: 0 0 0 0;font-size: 1.5em;padding: 5px 20px;" />Abort</button></a>
