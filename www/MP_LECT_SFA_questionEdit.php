...
if ($_GET["act"]=="reg" AND $qstId) {
	...
	if ($checkStatus) {
		...
		if ($check->isOk()) {
			...
			//$path=ereg_replace("MP_LECT_SFA_questionEdit.php","",$_SERVER["PATH_TRANSLATED"]);
		...
		$redirectUrl="./MP_LECT_SFA_questionManage.php?".$queryStr;

	} else $redirectUrl="./MP_LECT_SFA_questionEdit.php?msg=KO&".$queryStr;
...
//		  });
					...
					<form method="post" action="./MP_LECT_SFA_questionEdit.php?act=reg&id_act=<?=$qstId?>&<?=$queryStr?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
