...
if ($_GET["act"]=="reg" AND $vidId) {
	...
	if ($checkStatus) {
		...
		$redirectUrl="./MP_LECT_SNA_videoRevValidateResponse.php?id_act=".$vidId."&".$queryStr;

	} else $redirectUrl="./MP_LECT_SNA_videoRevValidateEdit.php?id_act=".$vidId."&msg=KO&".$queryStr;
...
//		  s = s.split('\'').join('&#92;');
		...
		function nl2br (str, is_xhtml) {
		  ...
		  return (str + '').replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1' + breakTag + '$2');
					...
					<p class="rsvPage_Title">Validate Video Review</p>
					...
					<form method="post" action="./MP_LECT_SNA_videoRevValidateEdit.php?act=reg&id_act=<?=$vidId?>&<?=$queryStr?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
						...
						<div class="signup_submit" style="width: 225px;margin: 25px auto;">
							<a href="./MP_LECT_SNA_videoRevValidateResponse.php?id_act=<?=$vidId?>&<?=$queryStr?>"><button type="button" class="abort" style="font-size: 1.3em;padding: 5px 20px;" />Abort</button></a>
