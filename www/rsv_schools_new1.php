...
if ($_GET["com"]=="reg") {
	...
	if ($id_sch AND $qst02 AND $qst00) {
		$sql = "
		INSERT INTO `members_school_teacher`
		(`id_user` , `id_sch` ,`teach_name` , `teach_web` , `teach_email` , `teach_subject` , `teach_experience` , `teach_prev_exp`)
		VALUES
		('$qst00', '$id_sch','$qst02', '$qst03', '$qst04', '$qst05', '$qst06', '$qst07')";
	...
	$check= new CheckUpload($_FILES["fupl"]);
		if ($check->isOk()) {
			...
			$path=ereg_replace("rsv_school_new1.php","",$_SERVER["PATH_TRANSLATED"]);
		...
		if ($_POST['teacher']=="Another Teacher") {
			//Redirect su messaggio
			$strpas11="./rsv_schools_new1.php?id_sch=".$id_sch."&opz=".$opz;
...
<body>
	...
	<div class="container">
		...
		<hr>
					...
					<p class="titpar">SCHOOL PRESENTATION FORM 2/2</p>
					<div class="txt">
						Fill in information about the teacher then press "Send" button.<br />If you want to insert another teacher press "Another teacher" button.</a>
						<?php if ($opz!=1) {?>
							<br />If you have done <a href="./rsv_schools.php">click here</a><br />
							<!-- <br />If you want to <strong>add a counsellor</strong> in this school <a href="./rsv_schools_new2.php?id_sch=<?=$id_sch?>&opz=<?=$opz?>">click here</a> -->
						<?php } else {?>
							<br /><br />
					...
					<form method="post" action="./rsv_schools_new1.php?com=reg&id_sch=<?=$id_sch?>&opz=<?=$opz?>" onreset="return confirm('Are you sure you want to reset the document?')" enctype="multipart/form-data">
						...
						<p class="titpar"><?=strtoupper("Name of the teacher")?><br /><span style="font-size: 8pt;font-weight: normal;line-height: 1em;"></span></p>
						<div class="lbl023">
						...
						<div class="lbl021">
							<p style="float: left;width: 120px;margin-left: 25px;"><input type="radio" name="qst05" value="Humanistic Subjects" />&nbsp;Humanistic Subjects</p>
							<p style="float: left;width: 120px;margin-left: 20px;"><input type="radio" name="qst05" value="English" />&nbsp;English</p>
							<p style="float: left;width: 120px;margin-left: 20px;"><input type="radio" name="qst05" value="Other" />&nbsp;Other</p>
							<div class="clear"></div>
						</div> -->
						<!-- <div class="lbl023">
						...
						<br /><br />
						<div class="lbl04"><input type="submit" name="Send" value="Send" /> <input type="submit" name="teacher" style="background-color:#33FF00;" value="Another Teacher" /> <input type="reset" value="Reset" /></div>
