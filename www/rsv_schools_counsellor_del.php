...
if ($_GET["com"]=="reg" AND $id_sch AND $id_tch) {
	...
	$sql = "DELETE FROM `members_school_counsellor` WHERE id_tch=".$id_tch;
$result=mysqli_query($conn, $sql);


	$file_del="./data/schools/counsellor/{$id_sch}_{$id_tch}.jpg";
	...
	while ($row=mysqli_fetch_array($result)) {
		$couns_name=$row["couns_name"];
		$couns_web=$row["couns_web"];
		$couns_email=$row["couns_email"];
		$couns_specialization=$row["couns_specialization"];
		$couns_experience=$row["couns_experience"];
	}
}
		...
		<div id="cnt1">
			<div id="corpo"><div id="rsv">
				...
				<div id="rsv_crp">
					...
					<div class="el_msg">
						<p class="p01">Do you want to delete this Counsellor?</p>

						<p class="titpar">NAME OF THE COUNSELLOR?</p>
						<p class="p03"><?=$couns_name?></p>
						<p class="titpar">NAME OF THE SCHOOL</p>
						<p class="p03"><?=$sch_name?></p>

						<div style="margin: 35px 0 0 0;">
							<a href="./rsv_schools_counsellor_del.php?com=reg&id_sch=<?=$id_sch?>&id_tch=<?=$id_tch?>"><img src="./impianto/img/confirm.png" width="122" height="37" alt="" border="0" /></a>
							&nbsp;&nbsp;&nbsp;
							<a href="./rsv_schools_counsellor.php?id_sch=<?=$id_sch?>"><img src="./impianto/img/abort.png" width="122" height="37" alt="" border="0" /></a>
						</div>
					</div>
