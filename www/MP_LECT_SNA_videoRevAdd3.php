	...
	<script src='https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.5/latest.js?config=TeX-MML-AM_CHTML' async></script>
					...
					<?php if ($_GET["msg"]=="KO") {?>
						<div style="margin: 10px 0 25px 0;padding: 10px;font-size: 2.1em;color: #f00;text-align: center;border: solid 1px #f00;border-radius: 5px;">
							<p>In order to proceed, you need to select at least one question.</p>
						</div>
					<?php }?>

					<form method="post" action="./MP_LECT_SNA_videoRevAdd3.php?act=reg&idVideo=<?=$vidId?>&from=<?=$fromPage?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 15px;border: solid 1px #00aeef;border-radius: 10px;">
...
//								$resultP=mysqli_query($conn,$sqlP);
						...
						<!-- <div class="signup_submit" style="padding-left: 70px;">
							<a href="./MP_LECT_SNA_videoRevManage.php"><button type="button" class="abort" />Abort</button></a>
						...
						<?php if ($fromPage=="edit") {?>
							<div class="signup_submit" style="width: 240px;margin: 25px auto;">
								<a href="./MP_LECT_SNA_videoRevEdit.php?id_act=<?=$vidId?>"><button type="button" class="abort" style="margin: 0 0 0 0;font-size: 1.5em;padding: 5px 20px;" />Abort</button></a>
						...
						<?php } else {?>
							<div class="signup_submit" style="width: automargin: 25px auto;">
