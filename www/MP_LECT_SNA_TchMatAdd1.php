...
//	$subTopic=Pulisci_INS($_POST["subtopic"]);
	...
	if ($checkStatus) {
		...
		else $redirectUrl="./MP_LECT_SNA_TchMatAdd2.php?idVideo=".$vidId; // passa alla scelta delle keywords 

	} else $redirectUrl="./MP_LECT_SNA_TchMatAdd1.php?idVideo=".$vidId."&msg=KO&from=".$fromPage;
...
?>
					...
					<form method="post" action="./MP_LECT_SNA_TchMatAdd1.php?act=reg&idVideo=<?=$vidId?>&from=<?=$fromPage?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
						...
						<?php if ($fromPage=="edit") {?>
							<div class="signup_submit" style="width: 240px;margin: 25px auto;">
								<a href="./MP_LECT_SNA_TchMatEdit.php?from=<?=$fromPage?>&id_act=<?=$vidId?>"><button type="button" class="abort" style="margin: 0 0 0 0;font-size: 1.5em;padding: 5px 20px;" />Abort</button></a>
