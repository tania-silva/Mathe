...
if ($_GET["act"]=="reg" AND $vidId) {
	...
	if ($checkStatus) {
		...
		if ($check->isOk()) {
			...
			//$path=ereg_replace("MP_LECT_SNA_videoRevEdit.php","",$_SERVER["PATH_TRANSLATED"]);
			$upload_dir = "./data/mathePlatform/SNA/vdReviews"; 
	...
	} else $redirectUrl="./MP_LECT_SNA_videoRevEdit.php?msg=KO&".$queryStr;
...
//		  });
					...
					<p class="rsvPage_Title">Edit Video Review</p>
					...
					<form method="post" action="./MP_LECT_SNA_videoRevEdit.php?act=reg&id_act=<?=$vidId?>&<?=$queryStr?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">

						<p style="padding: 0 0 0 25px;">If you want to edit the Topic(s)/Subtopic(s) for this Video Review <a href="./MP_LECT_SNA_videoRevAdd1.php?idVideo=<?=$vidId?>&from=edit">click here</a>.</p>
						<p style="padding: 0 0 0 25px;">If you want to edit the Keyword(s) for this Video Review <a href="./MP_LECT_SNA_videoRevAdd2.php?idVideo=<?=$vidId?>&from=edit">click here</a>.</p>
						<!-- <p style="padding: 0 0 0 25px;">If you want to edit the Question(s) for this Video Review <a href="./MP_LECT_SNA_videoRevAdd3.php?idVideo=<?=$vidId?>&from=edit">click here</a>.</p> -->
						...
						<div class="signup_field_ext">
							<label style="font-weight: 400;color: #c00;">* Link of the Video</label>
							<span style="font-size: 8pt;font-weight: normal;line-height: 1em;">(Please indicate the last 11 characters of the YouTube link. If the video is not on YouTube, please insert the Internet address)</span>
						...
						<div class="signup_submit" style="width: auto;padding-left: 30px;">
							<a href="./MP_LECT_SNA_videoRevManage.php?<?=$queryStr?>"><button type="button" class="abort" style="width: 100px;font-size: 1.3em;padding: 5px 20px;" />Exit</button></a>
