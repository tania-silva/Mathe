...
if ($_GET["act"]=="reg" AND $qstId) {
	...
	if ($checkStatus) {
		...
		if ($check->isOk()) {
			...
			//$path=ereg_replace("MP_LECT_SFA_questionValidateEdit.php","",$_SERVER["PATH_TRANSLATED"]);
		...
		$redirectUrl="./MP_LECT_SFA_questionValidateResponse.php?id_act=".$qstId."&".$queryStr;

	} else $redirectUrl="./MP_LECT_SFA_questionValidateEdit.php?id_act=".$qstId."&msg=KO&".$queryStr;
...
//		  s = s.split('\'').join('&#92;');
		...
		function nl2br (str, is_xhtml) {
		  ...
		  return (str + '').replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1' + breakTag + '$2');
					...
					<form method="post" action="./MP_LECT_SFA_questionValidateEdit.php?act=reg&id_act=<?=$qstId?>&<?=$queryStr?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
						...
						<div class="signup_submit" style="padding-left: 70px;">
							<a href="./MP_LECT_SFA_questionValidateResponse.php?id_act=<?=$qstId?>&<?=$queryStr?>"><button type="button" class="abort" />Abort</button></a>
