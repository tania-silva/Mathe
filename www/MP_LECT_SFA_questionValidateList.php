...
//					$where.=")";
							...
							if ($srcTopic) {
								...
								foreach( $tpoSTR_Ar as $valore) {  
									...
									if ($tpo=="sub") {
										...
										if ($strSubtop_ar[0]==$srcTopic) {
											$where.="p.subtopic=$strSubtop_ar[1] OR ";
											$checkCicle=1;
										}
									}
								}
								...
								foreach( $tpoSTR_Ar as $valore) {  
									...
									if ($tpo=="top") {
										$strTopID=substr($valore,3);
										if ($strTopID) {
											$where.="(p.topic=$strTopID AND p.subtopic=0) OR ";
											$checkCicle=1;
										}
									}
									if ($tpo=="sub") {
										...
										if ($strSubtop) {
											$where.="(p.topic=$strSubtop_ar[0] AND p.subtopic=$strSubtop_ar[1]) OR ";
											$checkCicle=1;
										}
									}
								}
					...
					<form method="post" action="./MP_LECT_SFA_questionValidateList.php" style="padding: 10px;font-size: 0.9em;background-color: #e1f7ff;border: solid 1px #00AEEF;border-radius: 5px;">
						...
						<div style="float: left;width: 240px;">
							Topic:<br />
							<select name="srcTopic" onchange="subTopic32(this.options[this.selectedIndex].value,'<?=$tpoSTR?>');">
						...
						<a href="./MP_LECT_SFA_questionValidateList.php"><button type="button" class="abort" style="margin-left: 190px;" />All</button></a>
					...
					<?php if ($totale>0) {?>
						<table class="table table-hover">
							...
							while ($row=mysqli_fetch_array($resultP)) { 
								...
								$lnk1="./MP_LECT_SFA_questionValidateEdit.php?id_act=".$qstId."&".$queryStr;
								$lnk3="./MP_LECT_SFA_questionValidateResponse.php?id_act=".$qstId."&".$queryStr;
