...
if ($_GET["act"]=="reg" AND $qstId) {
	...
	if ($checkStatus) {
		...
		if ($check->isOk()) {
			...
			//$path=ereg_replace("MP_LECT_SFA_assessmentQstEdit.php","",$_SERVER["PATH_TRANSLATED"]);
			$upload_dir = "./data/mathePlatform/SFA/assAttach"; 
	...
	} else $redirectUrl="./MP_LECT_SFA_assessmentQstEdit.php?assId=".$assId."&qstNb=".$qstNb."&qstId=".$qstId."&msg=KO";
...
//		  s = s.split('\'').join('&#92;');
		...
		//		  });
					...
					<form method="post" action="./MP_LECT_SFA_assessmentQstEdit.php?act=reg&qstId=<?=$qstId?>&qstNb=<?=$qstNb?>&assId=<?=$assId?>" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
						...
						<div class="signup_submit" style="padding-left: 200px;">
							<a href="./MP_LECT_SFA_assessmentNew1.php?assId=<?=$assId?>"><button type="button" class="abort" style="width: 100px;padding: 5px;" />Exit</button></a>
							<input type="submit" name="save" value="SAVE" class="proceed" style="width: 150px;margin-left: 30px;padding: 5px;" />
						</div>
