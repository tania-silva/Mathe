...
if ($_GET["act"]=="reg" AND $usrId) {
	...
	if ($checkStatus) {
		...
		$sql = "
			INSERT INTO `platform__SNA__VID_reviews` 
			(`id_lect`, `title`, `author`, `description`, `link`, `languages`, `date`, `validate`)  
			VALUES ('$usrId', '$videoTitle', '$videoAuthor', '$videoDescription', '$videoLink', '$videoLang', '$date', '$validateValue')";
		$result=mysqli_query($conn,$sql);
		...
		if ($check->isOk()) {
			...
			//$path=ereg_replace("MP_LECT_SNA_videoRevAdd.php","",$_SERVER["PATH_TRANSLATED"]);
		...
		$redirectUrl="./MP_LECT_SNA_videoRevAdd1.php?idVideo=".$videoId;

	} else $redirectUrl="./MP_LECT_SNA_videoRevAdd.php?msg=KO";
...
//		  s = s.split('\'').join('&#92;');
		...
		//doPreview('init');
					...
					<p class="rsvPage_Title">Insert New Video Review</p>
					...
					<form method="post" action="./MP_LECT_SNA_videoRevAdd.php?act=reg" enctype="multipart/form-data" style="display: block;margin-top: 5px;padding: 20px 0 20px 50px;border: solid 1px #00aeef;border-radius: 10px;">
						...
						<div class="signup_field_ext">
							...
							<textarea name="videoLink" id="videoLink" style="height: 50px;" required /></textarea>
						</div>
