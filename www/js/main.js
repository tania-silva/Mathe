...
(function($) {
    ...
    $('.sub-menu-mobile').click(function(){
        var status=$(this).find('+ .sub-menu').data('opened');
        if (status) {
            $(this).find('+ .sub-menu').slideUp();
            $(this).find('+ .sub-menu').data('opened',0);
        } else{
            $(this).find('+ .sub-menu').slideDown();
            $(this).find('+ .sub-menu').data('opened',1);
        }
    })
    ...
    $('#calendar').fullCalendar({
        ...
        eventRender: function (event, element, view) { 
            ...
            $(view.el[0]).find('.fc-day[data-date=' + dateString + ']').css('background-color', '#d15700');
